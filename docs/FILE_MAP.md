...
| `III_SurfReg2.sh` | `bin/06_surfreg_pass2.sh` | the icosphere resampling is now one `mri_surf2surf --sval-xyz/--tval-xyz` call instead of a MATLAB round-trip through three per-axis `.curv` overlays; MATLAB is no longer needed anywhere |
...
| `IV_sampling.sh` | `bin/07_sampling.sh` | dropped the MATLAB `obj_convert` step — coordinates are read straight from the resampled surface; dropped the second, redundant `flirt` call (it re-applied the matrix the first call had just produced, with identical output); dead `dicom2nifti` lines removed |
...
| `obj_convert.m`, `ResampleMesh2Icosphere_FreeSurfer.m` | the MATLAB icosphere resampling, replaced by a single `mri_surf2surf` call in `bin/06_surfreg_pass2.sh`. The originals depended on an in-house toolbox that was never publicly available, and their authorship was unresolved — see `NOTICE`. |
